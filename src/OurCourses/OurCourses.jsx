...
import { FaStar, FaClock, FaUser, FaTag, FaShareAlt, FaCheck } from "react-icons/fa"; // FaShareAlt add kiya
...
const OurCourses = () => {
  ...
  const [copiedId, setCopiedId] = useState(null); // Copy feedback k liye
  ...
  // Page load hote hi check karega k URL mein course ID hai ya nahi
  useEffect(() => {
  ...
  const handleCopyLink = (e, id) => {
    e.stopPropagation(); // Card click event ko rokne k liye
    const baseUrl = window.location.origin + window.location.pathname;
    const shareableLink = `${baseUrl}?course=${id}`;
    
    navigator.clipboard.writeText(shareableLink).then(() => {
      setCopiedId(id);
      setTimeout(() => setCopiedId(null), 2000); // 2 second baad icon reset
    });
  };
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 -mt-24 pb-20 relative z-20">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6 md:gap-10">
          {courses.map((course, index) => {
            const studentCount = (course.id * 9) % 35 + 15;
              ...
              >
                <div className="relative h-32 sm:h-52 md:h-60 overflow-hidden">
                  ...
                  <button 
                    ...
                    className="absolute bottom-2 right-2 bg-white/90 p-2 rounded-full text-[#265336] hover:bg-emerald-500 hover:text-white transition-all shadow-md z-30"
                    title="Copy Course Link"
                  >
                    {copiedId === course.id ? <FaCheck size={12} /> : <FaShareAlt size={12} />}
                  </button>
      ...
      {selectedCourse && !showForm && (
        <CourseDetails
          ...
          onClose={() => {
            ...
            // Link close hone par URL saaf karne k liye (Optional)
