...
const QuantityTabs = ({ tierPrices }) => {
  ...
  return (
    <div className="mb-6">
      <div className="flex justify-center md:justify-start mb-3 flex-wrap text-[13px] sm:text-lg gap-[4px] sm:gap-2">
        {priceKeys.map((range) => (
          <button
            key={range}
            onClick={() => setSelectedRange(range)}
            className={`px-3 py-2 sm:px-4 rounded-full border-2 font-semibold transition-all duration-300 cursor-pointer ${
              selectedRange === range
      ...
      {selectedRange && (
        <p className="text-medium sm:text-lg font-bold text-[#265336]">
